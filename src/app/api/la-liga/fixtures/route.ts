import { NextRequest, NextResponse } from "next/server";
import { validateGetQuery } from "@/lib/api/response";
import { emptyQuerySchema } from "@/lib/validation/schemas";
import { captureRouteError, logInfo } from "@/lib/log";
import {
  fetchLaLigaFixtures,
  laLigaFixturesCacheControl,
} from "@/lib/la-liga/api";
import { getCached, setCached } from "@/lib/server/cache";

export const dynamic = "force-dynamic";

const ROUTE = "/api/la-liga/fixtures";
const CACHE_KEY = "la-liga-fixtures";

export async function GET(request: NextRequest): Promise<NextResponse> {
  const validated = validateGetQuery(request, emptyQuerySchema);
  if ("error" in validated) return validated.error;

  const cached = getCached(CACHE_KEY);
  if (cached) {
    logInfo(ROUTE, "CACHE HIT");
    return NextResponse.json(cached, {
      headers: {
        "Cache-Control": laLigaFixturesCacheControl(
          cached as Parameters<typeof laLigaFixturesCacheControl>[0],
        ),
      },
    });
  }

  logInfo(ROUTE, "CACHE MISS");

  try {
    const body = await fetchLaLigaFixtures();
    setCached(CACHE_KEY, body);

    return NextResponse.json(body, {
      headers: {
        "Cache-Control": laLigaFixturesCacheControl(body),
      },
    });
  } catch (error) {
    captureRouteError("api/la-liga/fixtures", error);

    return NextResponse.json(
      {
        configured: Boolean(process.env.API_FOOTBALL_KEY?.trim()),
        league: "La Liga",
        leagueId: 140,
        season: 2026,
        fixtures: [],
        source: "fallback",
        fetchedAt: new Date().toISOString(),
        error: "Failed to fetch La Liga fixtures.",
      },
      {
        status: 500,
        headers: { "Cache-Control": "no-store" },
      },
    );
  }
}
