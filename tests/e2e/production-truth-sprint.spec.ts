...
test.describe("Production truth remediation sprint", () => {
  test("homepage prioritises PL and does not feature stale WC26 match", async ({
    page,
  }) => {
    ...
    await expect(leagues.getByText("World Cup 2026 Archive")).toBeVisible();
    await expect(page.getByText("Brazil")).not.toBeVisible();
  });

  test("live centre is competition-neutral after WC26 archive", async ({
    page,
  }) => {
    await preparePage(page);
    ...
    await expect(
      page.getByRole("main").getByRole("link", { name: "World Cup 2026 Archive" }),
    ).toBeVisible();
  });

  test("Premier League hub renders meaningful SSR shell content", async ({
    page,
  }) => {
    await preparePage(page);
  ...
  test("World Cup archive calendar defaults within tournament bounds", async ({
    page,
  }) => {
    await preparePage(page);
  ...
  test("completed archived match avoids permanent loading header", async ({
    page,
  }) => {
    await preparePage(page);
  ...
  test("mobile bottom navigation remains usable", async ({ page }) => {
    ...
    const liveTab = mobileNav.getByRole("link", { name: /^Live$/i });
